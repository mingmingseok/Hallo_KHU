...
def simplify_reference_unet(reference_unet):
    # conv_in과 conv_out만 남기고 나머지 계층 제거
    simplified_unet = nn.Sequential(
        reference_unet.conv_in,  # 기존 conv_in 사용
        nn.Conv2d(
            in_channels=reference_unet.conv_in.out_channels,  # conv_in 출력 채널
            out_channels=reference_unet.conv_out.out_channels,  # 최종 출력 채널
            kernel_size=1,  # 1x1 컨볼루션으로 입력과 출력 간 매핑
        )
    )
    return simplified_unet
...
def reduce_channels(unet_model, factor=2):
    ...
    for block in unet_model.down_blocks + unet_model.up_blocks:
        for layer in block.resnets:
            layer.conv1 = nn.Conv2d(
                in_channels=max(1, layer.conv1.in_channels // factor),  # 최소 1 보장
                out_channels=max(1, layer.conv1.out_channels // factor),
                kernel_size=layer.conv1.kernel_size,
                stride=layer.conv1.stride,
                padding=layer.conv1.padding,
            )
            layer.conv2 = nn.Conv2d(
                in_channels=max(1, layer.conv2.in_channels // factor),  # 최소 1 보장
                out_channels=max(1, layer.conv2.out_channels // factor),
    ...
    # conv_in 수정
    unet_model.conv_in = nn.Conv2d(
        in_channels=unet_model.conv_in.in_channels // factor,
        out_channels=unet_model.conv_in.out_channels // factor,
    ...
    # conv_out 수정
    # unet_model.conv_norm_out = nn.Conv2d(
    #     in_channels=unet_model.conv_norm_out // factor,
    #     out_channels=unet_model.conv_norm_out.out_channels,
    #     kernel_size=unet_model.conv_norm_out.kernel_size,
    #     stride=unet_model.conv_norm_out.stride,
    #     padding=unet_model.conv_norm_out.padding,
    # )
...
def train_stage1_process(cfg: argparse.Namespace) -> None:
    ...
    if accelerator.is_main_process:
        init_output_dir([save_dir, module_dir, validation_dir])
    ...
    student_reference_unet = reduce_channels(reference_unet, factor=2)
    student_denoising_unet = reduce_channels(denoising_unet, factor=2)


    net.reference_unet.conv_in = nn.Conv2d(
        in_channels=4,  # 기존 2에서 4로 수정
        out_channels=net.reference_unet.conv_in.out_channels,
        kernel_size=net.reference_unet.conv_in.kernel_size,
        stride=net.reference_unet.conv_in.stride,
        padding=net.reference_unet.conv_in.padding,
    ).to(device="cuda", dtype=weight_dtype)

    # Denoising UNet 수정
    net.denoising_unet.conv_in = nn.Conv3d(
        in_channels=4,  # 기존 2에서 4로 수정
        out_channels=net.denoising_unet.conv_in.out_channels,
        kernel_size=net.denoising_unet.conv_in.kernel_size,
        stride=net.denoising_unet.conv_in.stride,
        padding=net.denoising_unet.conv_in.padding,
    ).to(device="cuda", dtype=weight_dtype)

    net.denoising_unet.conv_out = nn.Conv3d(  # InflatedConv3d로 수정
        in_channels=net.denoising_unet.conv_out.in_channels // 2,
        out_channels=net.denoising_unet.conv_out.out_channels,
        kernel_size=net.denoising_unet.conv_out.kernel_size,
        stride=net.denoising_unet.conv_out.stride,
        padding=net.denoising_unet.conv_out.padding,
    )
    ...
    student_reference_control_writer = ReferenceAttentionControl(
        student_reference_unet,
        do_classifier_free_guidance=False,
        mode="write",
        fusion_blocks="full",
    ...
    student_reference_control_reader = ReferenceAttentionControl(
        student_denoising_unet,
        do_classifier_free_guidance=False,
        mode="read",
        fusion_blocks="full",
    ...
    logger.info(f"Loaded teacher model weights. Missing keys: {len(m)}, Unexpected keys: {len(u)}")
