...
class SimplifiedUNet(nn.Module):
    def __init__(self, original_unet, dtype=torch.float16):
        super(SimplifiedUNet, self).__init__()
        self.conv_in = original_unet.conv_in.to(dtype=dtype)
        self.down_blocks = nn.ModuleList([original_unet.down_blocks[0].to(dtype=dtype)])  # 첫 번째 Down 블록만 유지
        self.mid_block = original_unet.mid_block.to(dtype=dtype)  # 중앙 블록 유지
        self.up_blocks = nn.ModuleList([original_unet.up_blocks[0].to(dtype=dtype)])      # 첫 번째 Up 블록만 유지
        self.conv_out = (
            original_unet.conv_out.to(dtype=dtype) if hasattr(original_unet, "conv_out") else nn.Identity()
        )

    def forward(self, sample, *args, **kwargs):
        sample = sample.to(dtype=self.conv_in.weight.dtype)  # Ensure input is float
        # Remove unsupported 'return_dict' keyword from kwargs
        filtered_kwargs = {
            k: v.to(dtype=self.conv_in.weight.dtype) if isinstance(v, torch.Tensor) else v
            for k, v in kwargs.items() if k != "return_dict"
        }

        # Debug: Print shape after each step
        sample = self.conv_in(sample)
        print(f"After conv_in: {sample.shape}")

        for i, down_block in enumerate(self.down_blocks):
            print(f"down_block {i}: {down_block}")

        for i, down_block in enumerate(self.down_blocks):
            print(f"Before down_block {i}: sample.shape = {sample.shape}")
            print(f"Before down_block {i}: filtered_kwargs = {filtered_kwargs}")
            sample = down_block(sample, *args, **filtered_kwargs)
            print(f"After down_block {i}: sample.shape = {sample.shape}")


        for i, down_block in enumerate(self.down_blocks):
            sample = down_block(sample, *args, **filtered_kwargs)
            print(f"After down_block {i}: {sample.shape}")

        sample = self.mid_block(sample, *args, **filtered_kwargs)
        print(f"After mid_block: {sample.shape}")

        for i, up_block in enumerate(self.up_blocks):
            sample = up_block(sample, *args, **filtered_kwargs)
            print(f"After up_block {i}: {sample.shape}")
...
def simplify_reference_unet(reference_unet):
    simplified_down_blocks = reference_unet.down_blocks[:2]
    simplified_up_blocks = reference_unet.up_blocks[:2]

    # Transformer 블록 축소
    for block in simplified_down_blocks + simplified_up_blocks:
        # CrossAttnUpBlock2D와 CrossAttnDownBlock2D만 처리
        if hasattr(block, "attentions"):
            for attn in block.attentions:
                attn.transformer_blocks = attn.transformer_blocks[:1]

    # 중앙 블록 제거
    simplified_mid_block = None  # 필요시 남기기

    return nn.Sequential(
        reference_unet.conv_in,
        *simplified_down_blocks,
        simplified_mid_block,
        *simplified_up_blocks,
        reference_unet.conv_out if hasattr(reference_unet, "conv_out") else nn.Identity()
    )

def simplify_denoising_unet(denoising_unet):
    """
    Denoising UNet3D 간소화 함수.
    첫 두 개의 down_blocks와 up_blocks만 유지하며, transformer_blocks를 축소.
    """
    simplified_down_blocks = denoising_unet.down_blocks[:2]
    simplified_up_blocks = denoising_unet.up_blocks[:2]

    # Transformer 블록 축소
    for block in simplified_down_blocks + simplified_up_blocks:
        if hasattr(block, "attentions"):
            for attn in block.attentions:
                attn.transformer_blocks = attn.transformer_blocks[:1]  # transformer 블록 축소

    # 중앙 블록 간소화 (필요한 경우)
    simplified_mid_block = None  # 필요 시 유지

    return nn.Sequential(
        denoising_unet.conv_in,
        *simplified_down_blocks,
        simplified_mid_block,
        *simplified_up_blocks,
        denoising_unet.conv_out if hasattr(denoising_unet, "conv_out") else nn.Identity()
    )


def reduce_channels(unet_model, factor=2):
    """
    Reduce the number of channels in the UNet model by a given factor.

    Args:
        unet_model (nn.Module): The UNet model to modify.
        factor (int): The factor by which to reduce the number of channels.

    Returns:
        nn.Module: The modified UNet model.
    """
    for block in unet_model.down_blocks + unet_model.up_blocks:
        for layer in block.resnets:
            layer.conv1 = nn.Conv2d(
                in_channels=layer.conv1.in_channels // factor,
                out_channels=layer.conv1.out_channels // factor,
                kernel_size=layer.conv1.kernel_size,
                stride=layer.conv1.stride,
                padding=layer.conv1.padding,
            )
            layer.conv2 = nn.Conv2d(
                in_channels=layer.conv2.in_channels // factor,
                out_channels=layer.conv2.out_channels // factor,
                kernel_size=layer.conv2.kernel_size,
                stride=layer.conv2.stride,
                padding=layer.conv2.padding,
            )
    return unet_model

def reduce_resnet_blocks(unet_model, factor=2):
    """
    Reduce the number of ResNet blocks in the UNet model.

    Args:
        unet_model (nn.Module): The UNet model to modify.
        factor (int): Number of ResNet blocks to retain in each block.

    Returns:
        nn.Module: The modified UNet model.
    """
    for block in unet_model.down_blocks + unet_model.up_blocks:
        block.resnets = nn.ModuleList(block.resnets[:factor])
    return unet_model

def reduce_latent_space(unet_model, factor=2):
    """
    Reduce the latent space dimension in the UNet model.

    Args:
        unet_model (nn.Module): The UNet model to modify.
        factor (int): The factor by which to reduce the latent space dimension.

    Returns:
        nn.Module: The modified UNet model.
    """
    unet_model.latent_channels = unet_model.latent_channels // factor
    return unet_model
...
def train_stage1_process(cfg: argparse.Namespace) -> None:
    ...
    student_reference_unet = SimplifiedUNet(reference_unet).to(device="cuda", dtype=weight_dtype)
    student_denoising_unet = SimplifiedUNet3D(denoising_unet).to(device="cuda", dtype=weight_dtype)
    net = StudentNet(
    ...
    for _ in range(first_epoch, num_train_epochs):
        ...
        for _, batch in enumerate(train_dataloader):
            with accelerator.accumulate(net):
                ...
                print(f"Teacher prediction dtype: {teacher_pred.dtype}")
